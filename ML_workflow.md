...
### 2.1 train_prepare / test_prepare
1. **Edge 2 (Bronze Layer):** 원본 csv 로드 후 `test_fraction` 기준으로 두 Lane을 물리적 분할합니다.
2. **Edge 3 (Silver Layer - Validate):** 데이터 품질 검증 및 시간축 확정을 수행합니다.
...
### 2.2 train_featurize / test_featurize
1. **X/Y 변수 정의 및 그룹핑:**
